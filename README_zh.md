...
- 🌍 **双语支持**: 支持中文和英文输出
- 🗂️ **剧集式管理**: 每个剧集都有专属文件夹，包含所有相关文件
- 🤖 **自动化API**: 支持批量处理和与其他工作流程集成的程序化访问
- 📁 **智能文件管理**: 自动整理输出文件，清晰的层次结构
- 🔄 **语言切换**: 选择您偏好的界面语言
...
| 命令 | 描述 |
|---------|-------------|
| `autopodlens` | 英文版24x7自动化服务（每小时自动处理） |
| `autopod` | 中文版24x7自动化服务（每小时自动处理） |
| `autopodlens --status` | 检查英文版自动化服务状态 |
| `autopod --status` | 检查中文版自动化服务状态 |
